...
describe('TodoReducer', () => {
  test('add adds a new todo and saves to localStorage', () => {
    const action = { type: 'add', payload: { title: 'Task' } };
    const result = TodoReducer([], action);
    expect(result).toHaveLength(1);
    expect(result[0]).toEqual(expect.objectContaining({ title: 'Task', isCompleted: false }));
    expect(JSON.parse(localStorage.getItem('todos'))).toEqual(result);
  });

  test('delete removes todo by id and updates localStorage', () => {
    const initial = [{ id: '1', title: 't', details: '', isCompleted: false }];
  ...
  test('update modifies todo fields and updates localStorage', () => {
    const initial = [{ id: '1', title: 'Old', details: 'd', isCompleted: false }];
    localStorage.setItem('todos', JSON.stringify(initial));
    const action = { type: 'update', payload: { id: '1', title: 'New', details: 'newd' } };
    const result = TodoReducer(initial, action);
    expect(result[0]).toEqual({ id: '1', title: 'New', details: 'newd', isCompleted: false });
    expect(JSON.parse(localStorage.getItem('todos'))).toEqual(result);
  });

  test('get returns todos from localStorage', () => {
    const stored = [{ id: '1', title: 'stored', details: '', isCompleted: false }];
